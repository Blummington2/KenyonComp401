...
optimizer = torch.optim.AdamW(net.parameters(), lr=0.0001) # classic optimizer for gradient descent
# Adding schedulers so that I can alter learning rate mid-run
scheduler = StepLR(optimizer, step_size=1500, gamma=0.99) # LR decay 
scheduler2 = StepLR(optimizer, step_size=2000, gamma=0.98) # alterate LR decay
...
# set domain size
H = math.sqrt(8*np.pi/3)
lower_bound = -2.5*np.pi/H
...
def bc(number_pts, lower_bound, upper_bound):
    ...
    x_l = lower_bound*np.ones(((int)(number_pts/6),1))
    y_l = np.random.uniform(low=lower_bound, high=upper_bound, size=((int)(number_pts/6),1))
    z_l = np.random.uniform(low=lower_bound, high=upper_bound, size=((int)(number_pts/6),1))

    ## I need points for the x=upper boundary
    x_r = upper_bound*np.ones(((int)(number_pts/6),1))
    y_r = y_l
    z_r = z_l

    ## I need points for the y=lower boundary
    x_ll = np.random.uniform(low=lower_bound, high=upper_bound, size=((int)(number_pts/6),1))
    y_ll = lower_bound*np.ones(((int)(number_pts/6),1))
    z_ll = np.random.uniform(low=lower_bound, high=upper_bound, size=((int)(number_pts/6),1))

    ## I need points for the y=upper boundary
    x_rr = x_ll
    y_rr = upper_bound*np.ones(((int)(number_pts/6),1))
    z_rr = z_ll

    ## I need points for the z=lower boundary
    x_lll = np.random.uniform(low=lower_bound, high=upper_bound, size=((int)(number_pts/6),1))
    y_lll = np.random.uniform(low=lower_bound, high=upper_bound, size=((int)(number_pts/6),1))
    z_lll = lower_bound*np.ones(((int)(number_pts/6),1))

    ## I need points for the z=upper boundary
    x_rrr = x_lll
    y_rrr = y_lll
    z_rrr = upper_bound*np.ones(((int)(number_pts/6),1))
...
def interior(number_pts):
    ...
    x_sampling = np.append(x_sampling, np.random.normal(loc=0, scale=sigma, size=((int)(number_pts/2),1)))
    y_sampling = np.append(y_sampling, np.random.normal(loc=0, scale=sigma, size=((int)(number_pts/2),1)))
    z_sampling = np.append(z_sampling, np.random.normal(loc=0, scale=sigma, size=((int)(number_pts/2),1)))

    return x_sampling, y_sampling, z_sampling, u_sampling


# Interior Points
num_pts_interior = 15000
...
interior_y_sampling = Variable(torch.from_numpy(y_sampling).float(), requires_grad=True).to(device).unsqueeze(1)
interior_z_sampling = Variable(torch.from_numpy(z_sampling).float(), requires_grad=True).to(device).unsqueeze(1)
interior_all_zeros = Variable(torch.from_numpy(u_sampling).float(), requires_grad=False).to(device) # tensor to be compared to # already corre ctshape so don't unsqueeze

num_pts_boundary = 3000
...
net.load_state_dict(torch.load('poisson_pre-pre-train_epoch=99000.pt'))
...
for epoch in range(iterations):
    ...
    if epoch % 10000 == 0:
        ...
        with torch.autograd.no_grad():
            ...
            torch.save(net.state_dict(), './poisson_pre-train_epoch=' + str(epoch) + '.pt')
    
    # Loss based on PDE
    f_out = f_linear(interior_x_sampling, interior_y_sampling, interior_z_sampling, net)  # output of f(x,t)
    ...
    """
    l_f = f_linear(x_l_sampling, y_l_sampling, z_l_sampling)
    r_f = f_linear(x_r_sampling, y_r_sampling, z_r_sampling)
    ll_f = f_linear(x_ll_sampling, y_ll_sampling, z_ll_sampling)
    rr_f = f_linear(x_rr_sampling, y_rr_sampling, z_rr_sampling)
    lll_f = f_linear(x_lll_sampling, y_lll_sampling, z_lll_sampling)
    rrr_f = f_linear(x_rrr_sampling, y_rrr_sampling, z_rrr_sampling)
    ...
    if epoch % 100 == 0:
        with torch.autograd.no_grad():
            writer1.writerow( [epoch,"Training Loss:",loss.item(), ", Learning rate:", optimizer.param_groups[0]['lr'], ", Number of training points (boundary):",(x_l_sampling.size(0) + x_ll_sampling.size(0) + x_lll_sampling.size(0) + x_r_sampling.size(0) + x_rr_sampling.size(0) + x_rrr_sampling.size(0)), ", Number of training points (interior):", interior_x_sampling.size(0), ", VRAM allocated:", torch.cuda.memory_allocated(), ", VRAM reserved:", torch.cuda.memory_reserved()])
            print(epoch,"Training Loss:",loss.item(), ", Learning rate:", optimizer.param_groups[0]['lr'], ", Number of training points (boundary):",(x_l_sampling.size(0) + x_ll_sampling.size(0) + x_lll_sampling.size(0) + x_r_sampling.size(0) + x_rr_sampling.size(0) + x_rrr_sampling.size(0)), ", Number of training points (interior):", interior_x_sampling.size(0), ", VRAM allocated:", torch.cuda.memory_allocated(), ", VRAM reserved:", torch.cuda.memory_reserved())

## Save my new model
torch.save(net.state_dict(), './poisson_pre-train.pt')
