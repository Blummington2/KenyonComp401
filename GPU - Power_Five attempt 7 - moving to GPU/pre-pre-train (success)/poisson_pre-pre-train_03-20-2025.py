...
optimizer = torch.optim.AdamW(net.parameters(), lr=0.00003) # classic optimizer for gradient descent
...
interior_all_zeros = Variable(torch.from_numpy(u_sampling).float(), requires_grad=False).to(device).unsqueeze(1) # tensor to be compared to
...
for epoch in range(iterations):
    ...
    # Loss based on PDE
    f_out = net(interior_x_sampling, interior_y_sampling, interior_z_sampling)
    sphere = 100*delta_rho(interior_x_sampling, interior_y_sampling, interior_z_sampling)  # output of f(x,t)
    mse_interior = mse_cost_function(f_out, sphere)
    ...
    if epoch % 100 == 0:
        with torch.autograd.no_grad():
            writer1.writerow( [epoch,"Training Loss:",loss.item(), ", Learning rate:", scheduler.get_last_lr()[0], ", Number of training points (boundary):",(x_l_sampling.size(0) + x_ll_sampling.size(0) + x_lll_sampling.size(0) + x_r_sampling.size(0) + x_rr_sampling.size(0) + x_rrr_sampling.size(0)), ", Number of training points (interior):", interior_x_sampling.size(0), ", VRAM allocated:", torch.cuda.memory_allocated(), ", VRAM reserved:", torch.cuda.memory_reserved()])
