...
scheduler = StepLR(optimizer, step_size=1000, gamma=0.80) # LR decay 
...
iterations = 800000 # Number of loops in training
...
for epoch in range(iterations): # 'Epoch' keeps track of current training iteration
    ...
    f_out_rel = f_relative(interior_x_sampling, interior_y_sampling, interior_z_sampling, net)  # Output of f(x,y,z)
    mse_interior = mse_cost_function(f_out_basic, interior_all_zeros) + 0.1 * mse_cost_function(f_out_rel, interior_all_zeros) # Automatically calculates interior loss with gradient disabled
