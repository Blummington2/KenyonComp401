...
class Net(nn.Module): # Net inherits the neural network behavior of nn.Module
    def __init__(self): # Class constructor with no additional aarguments
        ...
        sz = 128 # Set neurons per layer
        # Define the layers of the neural network
        self.hidden_layer1 = nn.Linear(3, sz)  
        self.hidden_layer2 = nn.Linear(sz, sz)  
        self.hidden_layer3 = nn.Linear(sz, 3*sz)  
        self.hidden_layer4 = nn.Linear(3*sz, sz)  
...
optimizer = torch.optim.AdamW(net.parameters(), lr=0.0008) 
# Adding schedulers so that I can alter learning rate mid-run
scheduler = StepLR(optimizer, step_size=1000, gamma=0.85) # LR decay 
...
def delta_rho(x, y, z): 
    ...
    return (1)*torch.exp(-0.5*r*r/sigma/sigma) # - 0.0056776 # Gaussian distribution
...
def f_relative(x,y,z, net): # Same as f(), but without nonlinear part
    ...
    pde = laplacian / u / u / u / u / u - 2 * math.pi * delta_rho(x,y,z) 
    safe_laplacian = torch.where(torch.abs(laplacian / u / u / u / u / u) < sqt_mach_eps, torch.tensor(sqt_mach_eps, device=device, dtype=laplacian.dtype), laplacian)
    
    return pde / (safe_laplacian / u / u / u / u / u)
...
iterations = 2000000 # Number of loops in training
...
for epoch in range(iterations): # 'Epoch' keeps track of current training iteration
    ...
    if epoch % 20000 == 0: # Every 10000 epochs...
    ...
    # Loss based on PDE
    f_out_basic = f(interior_x_sampling, interior_y_sampling, interior_z_sampling, net)
    mse_interior = mse_cost_function(f_out_basic, interior_all_zeros)  # Automatically calculates interior loss with gradient disabled
    ...
    if (400 < epoch < 19000) or (80000 < epoch < 100000):
