...
pt_x_sampling = Variable(torch.from_numpy(x_sampling).float(), requires_grad=True).to(device) # turns BC samples into PyTorch tensors with gradient enabled
pt_y_sampling = Variable(torch.from_numpy(y_sampling).float(), requires_grad=True).to(device)
pt_z_sampling = Variable(torch.from_numpy(z_sampling).float(), requires_grad=True).to(device)
pt_all_zeros = Variable(torch.from_numpy(u_sampling).float(), requires_grad=False).to(device) # tensor to be compared to
...
for epoch in range(iterations):
    ...
    # Loss based on PDE
    f_out = f(pt_x_sampling, pt_y_sampling, pt_z_sampling, net)  # output of f(x,t)
    mse_interior = mse_cost_function(f_out, pt_all_zeros)
