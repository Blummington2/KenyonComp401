...
device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu") # Enables GPU-accelerated computation with CUDA if available, otherwise computes on CPU
...
scheduler = StepLR(optimizer, step_size=100, gamma=0.993) # LR decay 
scheduler2 = StepLR(optimizer, step_size=1000, gamma=0.98) # alterate LR decay
...
def f(x,y,z, net): 
    ...
    pde = (u_xx + u_yy + u_zz) - 2 * math.pi * delta_rho(x,y,z) * u * u * u * u * u # Loss at any given point is defined in terms of the differential equation
    return pde
...
def f_relative(x,y,z, net): # Same as f(), but without nonlinear part
    u = net(x,y,z) 
    u_x = torch.autograd.grad(u.sum(), x, create_graph=True)[0] 
    u_xx = torch.autograd.grad(u_x.sum(), x, create_graph=True)[0]
    u_y = torch.autograd.grad(u.sum(), y, create_graph=True)[0]
    ...
    sqt_mach_eps = math.sqrt(torch.finfo(torch.float64).eps)

    pde = laplacian - 2 * math.pi * delta_rho(x,y,z) * u * u * u * u * u
    safe_laplacian = torch.where(torch.abs(laplacian) < sqt_mach_eps, torch.tensor(sqt_mach_eps, device=device, dtype=laplacian.dtype), laplacian)
    
    return pde / safe_laplacian
...
def bc(number_pts, lower_bound, upper_bound):
    ...
    x_l = lower_bound*np.ones(((int)(number_pts/6),1)) # Creates array [[lb],[lb],[lb],...,[lb]]
    y_l = np.random.uniform(low=lower_bound, high=upper_bound, size=((int)(number_pts/6),1)) # Random sampling within domain values
    z_l = np.random.uniform(low=lower_bound, high=upper_bound, size=((int)(number_pts/6),1)) # Random sampling within domain values

    ## I need points for the x=upper boundary
    x_r = upper_bound*np.ones(((int)(number_pts/6),1)) # Creates array [[ub],[ub],[ub],...,[ub]]
...
iterations = 300000 # Number of loops in training

checkpoint = torch.load('pre-train_checkpoint_epoch=990000.pth') # load pre-training model and (optionally) optimizer state
net.load_state_dict(checkpoint['net_state_dict'])
# optimizer.load_state_dict(checkpoint['optimizer_state_dict'])

#net.load_state_dict(torch.load('poisson_pre-train_epoch=99000.pt'))
...
for epoch in range(iterations): # 'Epoch' keeps track of current training iteration
    ...
    if epoch % 5000 == 0: # Every 10000 epochs...
    ...
    # Loss based on PDE
    f_out = f_relative(interior_x_sampling, interior_y_sampling, interior_z_sampling, net)  # Output of f(x,y,z)
    f_out_rel = f(interior_x_sampling, interior_y_sampling, interior_z_sampling, net)
    f_out_opposite = f(-interior_x_sampling, -interior_y_sampling, -interior_z_sampling, net)
    mse_interior = mse_cost_function(f_out, interior_all_zeros) + mse_cost_function(f_out_rel, interior_all_zeros) + 0.01 * mse_cost_function(f_out_opposite, f_out) # Automatically calculates interior loss with gradient disabled
    ...
    """
    loss = mse_interior + 0.2 * mse_boundary # Total loss
    ...
    if (epoch < 30000) or (60000 < epoch < 95000):
        scheduler.step() # Adjust learning rate gradually
    if (120000 < epoch < 900000):
