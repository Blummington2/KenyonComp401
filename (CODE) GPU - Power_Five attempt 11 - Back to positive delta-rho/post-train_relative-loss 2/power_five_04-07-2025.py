...
optimizer = torch.optim.AdamW(net.parameters(), lr=0.000008) 
# Adding schedulers so that I can alter learning rate mid-run
scheduler = StepLR(optimizer, step_size=1000, gamma=0.92) # LR decay 
...
def f_relative(x,y,z, net): # Same as f(), but without nonlinear part
    ...
    pde = laplacian - 2 * math.pi * delta_rho(x,y,z)
...
iterations = 200000 # Number of loops in training
...
for epoch in range(iterations): # 'Epoch' keeps track of current training iteration
    ...
    if (5000 < epoch < 30000) or (60000 < epoch < 85000):
        scheduler.step() # Adjust learning rate gradually
    if epoch > 120000 and epoch < 900000:
