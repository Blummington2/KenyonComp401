...
def delta_rho(x, y, z): 
    ...
    return (1/(sigma*math.sqrt(2*torch.pi)))*torch.exp(-0.5*r*r/sigma) # Gaussian distribution
...
def visualize_function(lb= lower_bound, ub= upper_bound, num_slices=5, domain_size=100, device='cpu',epoch = 100): # Graphs a few slices of the full box
    ...
    sample_x = Variable(torch.from_numpy(x).float(), requires_grad=True).to(device) # Converts to a torch tensor and then to a torch variable tensor
    sample_y = Variable(torch.from_numpy(y).float(), requires_grad=True).to(device) # Converts to a torch tensor and then to a torch variable tensor
    ...
    for z in z_slices:
        ...
        sample_z = Variable(torch.from_numpy(zzz).float(), requires_grad=True).to(device) # Converts to a torch tensor and then to a torch variable tensor
...
def visualize_x0_slice(lb=lower_bound,ub=upper_bound, domain_size=100,device='cpu', epoch=0):
    ...
    sample_y = Variable(torch.from_numpy(y).float(), requires_grad=False).to(device)
    sample_z = Variable(torch.from_numpy(z).float(), requires_grad=False).to(device)
    
    xx = 0.0*y # x=0 is constant because we are taking a 2D slice through the origin
    sample_x = Variable(torch.from_numpy(xx).float(), requires_grad=False).to(device)
...
interior_y_sampling = Variable(torch.from_numpy(y_sampling).float(), requires_grad=True).to(device).unsqueeze(1) # unsqueeze fixes dimesnions
