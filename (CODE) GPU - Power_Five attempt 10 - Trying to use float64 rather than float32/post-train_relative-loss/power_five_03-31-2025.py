...
optimizer = torch.optim.AdamW(net.parameters(), lr=0.00006) 
# Adding schedulers so that I can alter learning rate mid-run
scheduler = StepLR(optimizer, step_size=2000, gamma=0.98) # LR decay 
...
checkpoint = torch.load('pre-pre-train_checkpoint_epoch=1190000.pth') # load pre-training model and (optionally) optimizer state
