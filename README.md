...
- Attempt 7: Edited the code so that it would run correctly on the Vera-Rubin (HPC) GPU nodes. Did multiple test runs with pre-training. Implemented Gaussian distribution parameters that are relevent to the physics problem at hand. Found Success in 'train 2'. There were 30,000+ sampling points in the domain with an average MSE loss of order 10^(-8). We can also visually see that the solution has good radial symmetry, which is a feature that we expect from a good model.
