...
- Attempt 13 (WIP): Uses a delta-rho distribution similar to attempt 7, but with a larger magnitude, upon the requesyt of Kenyon Cosmolab members. Experiemented with a normal run and another run which used a combination of normal loss and relative loss. The normal run found moderate success with average MSE loss on the order of 10^(-6) with 10,000+ sampling points. The run with relative loss failed to converge to a low-loss solution once again. 
